"""
Topics Unlock Page - Shows unlock badges for topics
"""
...
class TopicsUnlockPage:
    """Display topic unlocks with simple badges"""
    ...
    def __init__(self, parent, game_data):
        ...
        self.window.title("Topic Unlocks")
    ...
    def setup_ui(self):
        """Setup the topics unlock interface"""
        # Title
        title_frame = tk.Frame(self.window, bg='#2b2b3c', height=50)
        title_frame.pack(fill='x')
        title_frame.pack_propagate(False)

        tk.Label(title_frame, text="TOPIC UNLOCKS",
        ...
        unlocked_topics = self.unlock_system.get_all_unlocked_topics()
        ...
        # Create badge grid (dynamic based on number of topics)
        row = 0
        col = 0
        max_cols = 8  # Increased columns to fit more
        ...
        for i, topic in enumerate(all_topics):
            ...
            if is_unlocked:
                # Show topic name - single label
                tk.Label(badge_frame, text=topic,
        ...
        # Update stats (already created above)
        unlocked_count = len([t for t in all_topics if t in unlocked_topics])
        tk.Label(stats_frame, text=f"Topics Unlocked: {unlocked_count}/{len(all_topics)}",
